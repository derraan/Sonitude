...
#    dis_diff = max(dis_near) - min(dis_far)
    spatial_info = {
        "angle_near": angle_near,
        "dis_near": dis_near,
        "angle_far":angle_far,
        "dis_far": dis_far
    }
    return metadata, mixture, gt, tgt_speakers, spatial_info
...
def resolve_sample_dir(test_dir: str, save_id: int):
    # Support multiple dataset naming conventions for sample folders.
    candidates = [
        f"{save_id:05d}",
        f"{save_id:06d}",
        str(save_id),
    ]
    for sample_name in candidates:
        sample_dir = os.path.join(test_dir, sample_name)
        if os.path.isdir(sample_dir):
            return sample_name, sample_dir

    # Keep a deterministic fallback path for a clear downstream error message.
    sample_name = f"{save_id:05d}"
    return sample_name, os.path.join(test_dir, sample_name)
...
def main(args: argparse.Namespace):
    if args.use_cuda:
        if torch.cuda.is_available():
            ...
            print(
                "WARNING: --use_cuda was set, but CUDA is unavailable in this environment. "
                "Falling back to CPU."
            )
            device = "cpu"
    else:
    ...
    for sample_dir in sample_dirs:
        ...
        if args.save_id >= 0:
            sample_name, sample_dir = resolve_sample_dir(args.test_dir, args.save_id)
        print(f"Sample: {sample_name}", sample_dir)

        num += 1
        # Load data
        metadata, mixture, gt, tgt_speakers, spatial_info = load_testcase(sample_dir, args)
        ...
        infp['room'] = metadata['room_info']
        infp["spatial"] = spatial_info

        print('Num speakers inside the bubble:', n_tgt_speakers)
        if n_tgt_speakers == 0:
