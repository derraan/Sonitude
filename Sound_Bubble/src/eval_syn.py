...
def main(args: argparse.Namespace):
    ...
    for sample_dir in sample_dirs:
        ...
        # mixture = mixture.to(device)
        # row["hubert"] = hubert(output, gt, mixture[0:1]).item()
        # row["wavlm"] = wavlm(output, gt, mixture[0:1]).item()
        # print("Hurbert Loss = ", row["hubert"], "wavlm Loss = ", row["wavlm"])
        records.append(row)
    ...
    if  args.save_id  < 0:
        ...
        info_path = os.path.join(args.output_dir, 'infos.json')
