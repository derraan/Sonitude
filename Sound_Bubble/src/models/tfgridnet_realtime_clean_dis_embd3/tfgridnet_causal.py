...
class FilmLayer(nn.Module):
    def __init__(self, D_in, D):
        ...
        self.weight = nn.Conv1d(D_in, self.D, 1)
        self.bias = nn.Conv1d(D_in, self.D, 1)
    ...
    def forward(self, x: torch.Tensor, embedding: torch.Tensor):
        ...
        w = self.weight(embedding).unsqueeze(3) # (B, D, F, 1)
        b = self.bias(embedding).unsqueeze(3) # (B, D, F, 1)
...
class Dis_Embed_Linear(nn.Module):
    def __init__(self, label_len, n_embed, n_freqs, dim = "freq"):
        super().__init__()
        ...
        if dim == "freq":
            self.dis_embedding = nn.Sequential(
            nn.Linear(label_len, n_freqs, bias = False),
            nn.LayerNorm(n_freqs))
        else:
            self.dis_embedding = nn.Sequential(
            nn.Linear(label_len, n_freqs*n_embed, bias = False),
            nn.LayerNorm(n_freqs*n_embed))     


    def forward(self, embed):
        # embed - BxL
        embed = self.dis_embedding(embed)
        #print(self.dim)
        if self.dim == "freq":
            # embed - BXF
            ...
            B, FE = embed.shape
            #print("all", self.n_embed, self.n_freqs)
            embed = embed.view(B, self.n_embed, self.n_freqs)
            # embed - BXEXF
        
        return embed


class Dis_Embed_Conv(nn.Module):
    def __init__(self, label_len, n_embed, n_freqs, n_in):
        super().__init__()
        self.n_embed = n_embed
        self.n_in = n_in
        self.n_freqs = n_freqs
        ...
        self.dis_embedding = nn.Sequential(
                nn.Linear(label_len, n_freqs*n_in, bias = False))
                #nn.LayerNorm(n_freqs*n_in))
        self.dis_norm = nn.LayerNorm(n_in)
        # self.dis_conv = nn.Conv1d(n_in, n_embed, 1)
    ...
    def forward(self, embed):
        ...
        embed = self.dis_embedding(embed)
        B, FE = embed.shape
        embed = embed.view(B, self.n_freqs, self.n_in)
        embed = self.dis_norm(embed)
        embed = torch.transpose(embed, 1, 2)
        # embed = self.dis_conv(embed)
        # embed - BxEXF
        return embed
...
class TFGridNet(AbsSeparator):
    ...
    def __init__(
        ...
        D_in = emb_dim
        if dis_type == "linear1":
            self.embed_net = Dis_Embed_Linear(label_len, emb_dim, n_freqs, dim = "freq")
            D_in = 1
        elif dis_type == "linear2":
            self.embed_net = Dis_Embed_Linear(label_len, emb_dim, n_freqs, dim = "all")
            D_in = emb_dim
        elif dis_type == "conv1":
            self.embed_net = Dis_Embed_Conv(label_len, emb_dim, n_freqs, 1)
            D_in = 1
        elif dis_type == "conv2":
            self.embed_net = Dis_Embed_Conv(label_len, emb_dim, n_freqs, 2)
            D_in = 2
        elif dis_type == "conv3":
            self.embed_net = Dis_Embed_Conv(label_len, emb_dim, n_freqs, 4)
            D_in = 4
        elif dis_type == "conv4":
            self.embed_net = Dis_Embed_Conv(label_len, emb_dim, n_freqs, 8)
            D_in = 8
    ...
    def forward(
        ...
        #print(input.shape)
        dis_embed = self.embed_net(dis_embed)
        ...
        for ii in range(self.n_layers):
            if ii > 0:
                ...
                batch = self.embeds[ii - 1](batch, dis_embed)
...
class GridNetBlock(nn.Module):
    ...
    def forward(self, x, init_state = None):
        ...
        if self.conv_lstm:
            ...
            intra_rnn = F.pad(intra_rnn, pad=(0, 3)) # TODO: FIX ME (Change this to kernel - 1)
            
            intra_rnn = intra_rnn[..., :self.n_freqs]
