...
class Net(nn.Module):
    ...
    def __init__(
                    ...
                    look_ahead = True,
                    fair_compare = False, loss_type = "sisdr", r = 0.0
                ):
        ''' 
        ...
        self.kernel_num = [2*n_mics]+kernel_num 
    ...
    def forward(self, raw, lens=None):
        inputs = raw['mixture']

        if self.look_ahead:
            pad_size = (self.win_len - 2*self.win_inc, self.win_inc)
        else:
            pad_size = (self.win_len - self.win_inc, 0)

        inputs = F.pad(inputs, pad_size)

        B, M, T = inputs.shape
        inputs = inputs.view(B*M, 1, T)
        ###  the multi mic

        specs = self.stft(inputs)
        # print("input spect: ", specs.shape)
        real = specs[:,:self.fft_len//2+1, :]
        imag = specs[:,self.fft_len//2+1:, :]

        BM, Ft, T = real.shape
        real = real.view(B, M, Ft, T)
        imag = real.view(B, M, Ft, T)

        ### recover the multi mic
        ...
        cspecs = cspecs[:,:,1:] ## remove the bias 
        '''
        ...
        for idx in range(len(self.decoder)):
            # print(out.shape, encoder_out[-1 - idx].shape)
        ...
        # print('decoder', out.size())
        # print('spec', mask_real.size(), mask_imag.size(), real.size(), imag.size())
        # print('abs: ', spec_mags.size(), spec_phase.size())
        ...
        out_wav = out_wav[:, self.win_len - self.win_inc: ]
        # print(out_wav.size())
        out_wav = out_wav.unsqueeze(1)
        # return out_spec,  out_wav
        return {'output': out_wav, 'next_state': None}
...
if __name__ == '__main__':
    ...
    # DCCRN-CL
    fs = 24000
    inputs =  torch.rand(10, 6, fs*3)
    net = DCCRN(rnn_units=256, win_len = 512, win_inc = 100, fft_len= 512, masking_mode='E',use_clstm=True,kernel_num=[64, 128, 256, 256, 256,256])
    outputs = net(inputs)[1]
    # loss = net.loss(outputs, labels, loss_mode='SI-SNR')
    # print(loss)
