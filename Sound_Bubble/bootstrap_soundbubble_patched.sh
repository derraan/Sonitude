#!/usr/bin/env bash
# =============================================================================
# bootstrap_soundbubble.sh — Sound Bubble Pi 5 clean-slate setup + pipeline launcher
# =============================================================================
...
# --- Pipeline tuning ---
HOPS_PER_IO=4                 # 4 = lower latency, 8 = more jitter tolerance
...
MAX_OUTPUT_AHEAD_SEC=0.10     # Pi-safe low-latency output backlog cap
TARGET_OUTPUT_AHEAD_SEC=0.04  # trim target after backlog drop
MODEL_RING_MS=64              # model-input backlog budget in ms
MAX_STEPS_PER_BLOCK=4         # prefer dropout over runaway catch-up on Pi 5
REQUIRE_SOXR=false            # set true to fail fast unless streaming SoXR is available
...
if missing:
    ...
    print("Run:  bash bootstrap_soundbubble.sh   (without --run-only) to reinstall")
    sys.exit(1)
...
if [[ "$MAX_LAT" != "unknown" ]]; then
  ...
  if (( MAX_LAT > 5000 )); then
    warn "  Max latency > 5 ms — expect audible risk under load; consider PREEMPT_RT and governor tuning"
  elif (( MAX_LAT > 1000 )); then
    warn "  Max latency > 1 ms — usable, but not in the low-jitter regime"
  fi
fi
...
if [ "$REQUIRE_SOXR" = true ]; then
  EXTRA_FLAGS="--require-soxr"
else
  EXTRA_FLAGS=""
fi
...
info "  soxr:    $([ "$REQUIRE_SOXR" = true ] && echo required || echo optional)"
echo ""

"$VENV_PY" edge/inference_pipeline.py \
  $MODEL_FLAGS \
  ...
  --max-output-ahead-sec "$MAX_OUTPUT_AHEAD_SEC" \
  --target-output-ahead-sec "$TARGET_OUTPUT_AHEAD_SEC" \
  --model-ring-ms "$MODEL_RING_MS" \
