...
bool Elapsed(const std::uint64_t now_ns, const std::uint64_t start_ns,
  ...
  return start_ns != 0U && now_ns >= start_ns &&
         (now_ns - start_ns) >= duration_ns;
}
} // namespace
...
void OwnVoiceDetector::configure(const OwnVoiceDetectorConfig &config,
  ...
  if (!model_.calibrated) {
    throw std::runtime_error(
        "enabled OVD requires a calibrated statistical OVTF model");
  }
  if (!std::isfinite(config_.activation_probability) ||
      !std::isfinite(config_.release_probability) ||
      config_.activation_probability <= 0.0F ||
      config_.activation_probability > 1.0F ||
      config_.release_probability < 0.0F ||
  ...
  if (config_.activation_hold_ns == 0U || config_.release_hold_ns == 0U ||
    ...
    throw std::runtime_error(
        "enabled OVD hold and stale windows must be non-zero");
  }
  ...
  for (std::size_t feature = 0; feature < kOwnVoiceFeatureCount; ++feature) {
    if (!std::isfinite(model_.feature_mean[feature]) ||
      ...
      throw std::runtime_error(
          "OVD model statistics must be finite with positive variances");
    }
    weight_sum += model_.feature_weight[feature];
  }
  if (!(weight_sum > 0.0F) || !std::isfinite(weight_sum)) {
    throw std::runtime_error(
        "OVD model must enable at least one finite feature weight");
  }
  state_.health = OwnVoiceHealth::ModelUnavailable;
  configured_ = true;
}

float OwnVoiceDetector::probability(
    const OwnVoiceObservation &observation) const noexcept {
  ...
  return std::clamp(
      static_cast<float>(std::exp(-0.5 * weighted_distance / weight_sum)), 0.0F,
      1.0F);
}

OwnVoiceState
OwnVoiceDetector::makeUnhealthy(const OwnVoiceHealth health,
...
OwnVoiceState OwnVoiceDetector::evaluate(const OwnVoiceObservation &observation,
  ...
  if (!configured_ || !config_.enabled) {
    return makeUnhealthy(configured_ ? OwnVoiceHealth::Disabled
                                     : OwnVoiceHealth::ModelUnavailable,
                         observation.observed_ns);
  }
  if (!observation.valid) {
    return makeUnhealthy(OwnVoiceHealth::InvalidObservation,
                         observation.observed_ns);
  }
  ...
  if (!state_.active) {
    ...
    if (current_probability >= config_.activation_probability) {
      ...
      if (Elapsed(observation.observed_ns, activation_candidate_ns_,
                  config_.activation_hold_ns)) {
        state_.active = true;
        activation_candidate_ns_ = 0;
      }
    } else {
    ...
    if (current_probability <= config_.release_probability) {
      ...
      if (Elapsed(observation.observed_ns, release_candidate_ns_,
                  config_.release_hold_ns)) {
        state_.active = false;
        release_candidate_ns_ = 0;
      }
    } else {
...
OwnVoiceState
OwnVoiceDetector::stateForTime(const std::uint64_t now_ns) noexcept {
  if (!configured_ || !config_.enabled) {
    return state_;
  }
