...
OwnVoiceFeatureExtractor::OwnVoiceFeatureExtractor(const std::size_t reference_mic_index)
    : reference_mic_index_(reference_mic_index)
{
  if (reference_mic_index_ >= audio::kMicChannels)
  {
    throw std::invalid_argument("own-voice feature reference microphone is out of range");
  }
}

OwnVoiceObservation
OwnVoiceFeatureExtractor::extract(const std::span<const audio::MicFrame> calibrated_input,
                                  ...
                                  const std::uint64_t observed_ns) const noexcept
{
  ...
  if (calibrated_input.empty() || calibrated_input.size() != own_reference.size())
  {
    return observation;
  }
  ...
  for (std::size_t frame_index = 0; frame_index < calibrated_input.size(); ++frame_index)
  {
    ...
    own_energy += static_cast<double>(own_sample) * static_cast<double>(own_sample);

    const float reference_sample = calibrated_input[frame_index][reference_mic_index_];
    ...
    for (std::size_t channel = 0; channel < audio::kMicChannels; ++channel)
    {
      ...
      energy[channel] += static_cast<double>(sample) * static_cast<double>(sample);
  ...
  observation.features[0] = static_cast<float>(
      std::sqrt((own_energy + kEnergyFloor) / (mean_channel_energy + kEnergyFloor)));
  ...
  for (std::size_t channel = 0; channel < audio::kMicChannels; ++channel)
  {
    ...
    observation.features[level_feature++] =
        static_cast<float>(0.5 * std::log((energy[channel] + kEnergyFloor) /
                                          (energy[reference_mic_index_] + kEnergyFloor)));
    const double denominator =
        std::sqrt((energy[channel] + kEnergyFloor) * (energy[reference_mic_index_] + kEnergyFloor));
    observation.features[correlation_feature++] =
        static_cast<float>(std::clamp(reference_cross[channel] / denominator, -1.0, 1.0));
  }
  ...
  for (const float feature : observation.features)
  {
    ...
    {
      return OwnVoiceObservation{.sequence = sequence, .observed_ns = observed_ns, .valid = false};
    }
  }
