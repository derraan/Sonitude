                               ...
                               const float required_correlation)
{
  if (!std::isfinite(measured_correlation) || !std::isfinite(required_correlation) ||
      required_correlation < 0.0F || required_correlation > 1.0F)
  {
    throw std::runtime_error("invalid correlation quality value");
  }
  if (measured_correlation < required_correlation)
  {
    std::ostringstream message;
    message << "calibration correlation rejected: channel=" << channel_id
            << " measured=" << measured_correlation << " required=" << required_correlation;
