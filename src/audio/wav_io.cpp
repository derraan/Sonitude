...
WavData ReadWavFromStream(std::istream& in, const std::uint64_t available_input)
{
  ...
  if (sample_rate > (std::numeric_limits<std::uint32_t>::max() /
                     static_cast<std::uint32_t>(expected_block_align)))
  {
    throw std::runtime_error("WAV sample rate and block alignment overflow byte rate");
  }
  const std::uint32_t expected_byte_rate =
      sample_rate * static_cast<std::uint32_t>(expected_block_align);
