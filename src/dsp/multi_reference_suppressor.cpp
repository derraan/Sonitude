...
void MultiReferenceSuppressor::configure(
    const MultiReferenceSuppressorConfig &config) {
  if (config.taps == 0U || config.taps > kMaxMultiReferenceTaps) {
    throw std::runtime_error(
        "multi-reference suppressor taps must be in [1, 32]");
  }
  if (!InUnitInterval(config.step_size) || config.step_size == 0.0F ||
      !InUnitInterval(config.leakage) ||
      !std::isfinite(config.coefficient_limit) ||
      config.coefficient_limit <= 0.0F ||
      !std::isfinite(config.max_attenuation_db) ||
      config.max_attenuation_db < 0.0F || config.max_attenuation_db > 24.0F) {
    throw std::runtime_error(
        "invalid multi-reference suppressor configuration");
  }
...
float MultiReferenceSuppressor::historyAt(
    const AdaptiveReference &reference, const std::size_t tap) const noexcept {
  const std::size_t index = (reference.cursor + kMaxMultiReferenceTaps - tap) %
                            kMaxMultiReferenceTaps;
...
void MultiReferenceSuppressor::update(AdaptiveReference &reference,
                                      const float error,
  ...
  for (std::size_t tap = 0; tap < config_.taps; ++tap) {
    ...
    reference.weights[tap] = std::clamp(updated, -config_.coefficient_limit,
                                        config_.coefficient_limit);
  }
}
...
bool MultiReferenceSuppressor::process(
    const std::span<float> target_reference,
  ...
  for (std::size_t frame = 0; frame < target_reference.size(); ++frame) {
    if (!std::isfinite(target_reference[frame]) ||
        !std::isfinite(own_reference[frame]) ||
  ...
  for (std::size_t frame = 0; frame < target_reference.size(); ++frame) {
    ...
    const std::array<float, kMaxInterferenceReferences> input{
        own_reference[frame], distractor_reference[frame]};
    ...
    for (std::size_t reference_index = 0; reference_index < references_.size();
         ++reference_index) {
      AdaptiveReference &reference = references_[reference_index];
      predicted[reference_index] =
          pushAndPredict(reference, input[reference_index]);
      for (std::size_t tap = 0; tap < config_.taps; ++tap) {
    ...
    for (std::size_t reference_index = 0; reference_index < references_.size();
         ++reference_index) {
      estimated_interference += control.cancellation_mix[reference_index] *
                                predicted[reference_index];
    }
    ...
    estimated_interference =
        std::clamp(estimated_interference, -max_removal, max_removal);
...
MultiReferenceSuppressor::coefficientEnergy() const noexcept {
  ...
  for (std::size_t reference_index = 0; reference_index < references_.size();
       ++reference_index) {
    for (std::size_t tap = 0; tap < config_.taps; ++tap) {
      energy[reference_index] += references_[reference_index].weights[tap] *
                                 references_[reference_index].weights[tap];
    }
  }
  return energy;
}
