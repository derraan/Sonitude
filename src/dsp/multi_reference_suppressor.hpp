...
// Bounded two-reference normalized adaptive canceller. configure() is
// slow-path; process() is allocation-free, lock-free, and exception-free.
//
...
public:
  ...
  bool process(std::span<float> target_reference,
  ...
  coefficientEnergy() const noexcept;
...
private:
  ...
  float historyAt(const AdaptiveReference &reference,
