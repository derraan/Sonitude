...
void OwnVoiceReferenceExtractor::configure(
    const OwnVoiceReferenceModel &model) {
  if (!model.calibrated) {
    throw std::runtime_error(
        "OwnVoiceReferenceExtractor requires a calibrated model");
  }
  ...
  if (!(l1_norm > 0.0F) || !std::isfinite(l1_norm)) {
    throw std::runtime_error(
        "own-voice reference weights must have non-zero finite norm");
  }
...
bool OwnVoiceReferenceExtractor::process(
