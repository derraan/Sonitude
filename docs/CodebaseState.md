...
### Implementation status snapshot (M4–M6)

```text
capture -> calibration -> beamformer -> mono-to-stereo -> ASRC -> playback
```
...
| Stage 5 | M5–M6 control | in_progress | ODAS adapter, source tracker, conversation state machine implemented |
...
| Suppression / limiter | Pending (M7) |
...
| **M7–M8**            | `pending`     | Suppression/limiter and latency measurement not yet integrated |
...
**Pending:** live ODAS soak on Pi hardware; M7 suppression/limiter; end-to-end state-machine transition harness under `tests/integration/`.
