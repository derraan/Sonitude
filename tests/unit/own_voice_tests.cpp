...
void TestContinuousReferenceAndFeatures() {
  ...
  Require(extractor.process(input, own_reference),
          "continuous own reference extraction failed");
  Require(std::all_of(own_reference.begin(), own_reference.end(),
                      [](const float value) {
                        return std::fabs(value - 0.25F) < 1.0e-6F;
                      }),
  ...
  Require(observation.valid,
          "finite aligned period should produce a valid observation");
  for (std::size_t index = 1;
       index <= sonitude::ovd::kRelativeLevelFeatureCount; ++index) {
...
sonitude::ovd::StatisticalOvtfModel
ModelFor(const sonitude::ovd::OwnVoiceObservation &observation) {
...
void TestDetectorHysteresisAndStaleFailsafe() {
  ...
  Require(!state.active &&
              state.health == sonitude::ovd::OwnVoiceHealth::Healthy,
          "first matching observation should start, not skip, activation hold");
  own.observed_ns = 110;
  state = detector.evaluate(own, 110);
  Require(state.active,
          "matching observations should activate after the configured hold");
  ...
  Require(state.active,
          "first mismatch should start, not skip, release hangover");
  external.observed_ns = 130;
  state = detector.evaluate(external, 130);
  Require(!state.active,
          "mismatch should release after the configured hangover");
...
void TestBoundedChannelsDropInsteadOfBlocking() {
  ...
  Require(observations.publish(observation),
          "queue should accept first observation");
  Require(observations.publish(observation),
          "queue should accept second observation");
  Require(observations.publish(observation),
          "queue should accept usable capacity");
  Require(!observations.publish(observation),
          "full OVD queue must drop instead of blocking");
  ...
  for (std::uint64_t generation = 1; generation <= 3; ++generation) {
    ...
    Require(states.publish(state),
            "state channel should accept complete state");
  }
...
void TestMultiReferenceBypassAndAdaptationStates() {
  ...
  Require(suppressor.process(primary, own, distractor, bypass),
          "bypass processing failed");
  Require(primary == original,
          "BYPASS must preserve the target path bit-for-bit");

  sonitude::dsp::MultiReferenceControl adapt{};
  adapt.state[0] = sonitude::dsp::AdaptationState::Adapt;
  adapt.cancellation_mix[0] = 1.0F;
  primary = original;
  Require(suppressor.process(primary, own, distractor, adapt),
          "own-reference adaptation failed");
  const float tail_mean =
      std::accumulate(primary.end() - 64, primary.end(), 0.0F) / 64.0F;
  Require(tail_mean < 0.50F,
          "calibrated correlated reference should reduce interference");
  Require(tail_mean > 0.17F,
          "attenuation safety floor should prevent destructive nulling");
  ...
  primary = original;
  Require(suppressor.process(primary, own, distractor, hold),
          "HOLD processing failed");
  ...
  primary = original;
  Require(suppressor.process(primary, own, distractor, decay),
          "DECAY processing failed");
