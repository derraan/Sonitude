...
int main()
{
  ...
  {
    const auto runtime =
        sonitude::app::LoadRuntimeConfigFromFile(std::string(SONITUDE_SOURCE_DIR) +
                                                 "/tests/fixtures/runtime_valid.yaml");
    Require(runtime.capture.sample_rate_hz > 0, "runtime fixture should load for integration smoke");
